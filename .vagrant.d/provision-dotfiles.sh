...
if [ ! -d .git ]; then
    ...
    git init -q
    git remote add origin git://github.com/samscholfield/dotfiles.git
    git remote set-url --push origin git@github.com:samscholfield/dotfiles.git
    git fetch -q origin
...
if [ -f ~/.ssh/sam.pub ]; then
    sudo cp -f ~/.ssh/sam.pub ~root/.ssh/authorized_keys
fi
